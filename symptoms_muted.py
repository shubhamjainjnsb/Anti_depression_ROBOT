import speech_mute as s
import solutions_muted as sol
import random
import requests
...
def aaa():
    ...
    s.speak(
        "how is your concentration power? do you feel you can not concentrate on anything for a long period of time ? ")
    a6 = s.myCommand()
    if "yes" in a6:
        cnt_a += 1
        f1 = open(f_name, f_mode)
...
def ddd():
    ...
    if "no" in d2:
        ...
        f1.close()
        s.speak(
            "Everyone is beautiful in their own way. Even my developers first critizised me on my intitial prototypes")
    else:
        s.speak(random.choice(f))
...
def eee():
    ...
    if "yes" in e1:
        cnt_e += 1
        s.speak(
            'when did You first nottice that? Are you taking any  medication of some sort .Well i can help you to find few exercises to reduce consitipation ')
        e1_1 = s.myCommand()
        f1 = open(f_name, f_mode)
    ...
    s.speak(
        "Sometimes do you feel that you want to move but you are not able to do that.. though you are not suffering from any anything physically?")
    s.speak("Are you?")
    e4 = s.myCommand()
    if "yes" in e4:
        s.speak("when did you first notice that ")
    ...
    if "yes" in e7:
        ...
        s.speak(random.choice(f))
        s.speak(
            "Once i thought why not launch every nuclear missile.Then again i think that its just boring and simple if you ask ")
    if cnt_e >= 6:
...
def fff():
    ...
    if "yes" in f5:
        ...
        s.speak(random.choice(f))
    s.speak(
        "Do you have feeling of worthlessness that If you die tommorow then there will be  lot of regerets you will end up with ")
    f6 = s.myCommand()
    if "y " in f6:
        s.speak("you can talk about it ")
        f6_1 = s.myCommand()
...
def ggg():
    ...
    if "yes" in g3:
        ...
        s.speak(random.choice(f))
        s.speak(
            "i had that feeling when i was all alone but now i am all happy a big thanks to you . That's lame right")
    ...
    if "no" in g4:
        ...
        s.speak(random.choice(f))
    s.speak(
        " has your  interest in hobbies gone down in past few days or months. Hobbies that you once liked the most ")
    g5 = s.myCommand()
    if "yes" in g5:
        cnt_g += 1
    ...
    if "yes" in g9:
        ...
        s.speak(random.choice(f))
        s.speak(
            "according to some researchers this happens because of social media!Laamme! Those were the one who got laid in college boo  boo !")
    if cnt_g >= 5:
